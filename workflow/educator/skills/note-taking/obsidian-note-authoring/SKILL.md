...
9. **Using `patch` on markdown tables** — the `patch` tool can corrupt Obsidian markdown table syntax, introducing double-pipe `||` artifacts on table rows. When editing tables (adding rows, updating cell content), prefer `write_file` to rewrite the entire file instead of `patch`. Reserve `patch` for non-table content (paragraphs, bullet lists, YAML frontmatter, headings) where it is reliable. Always verify table syntax with `read_file` after any table edit.
