...
- **Token-dump timeout:** iterating `set.tokens.map(t => ({name, value, resolvedValue, type}))` across ALL 14 token sets in one `execute_code` call TIMES OUT at the 30s MCP task limit when a set has 300+ tokens (Foundations - Colors alone is ~300). Never dump all tokens at once — use `penpotUtils.tokenOverview()` (returns names only, fast) plus a small `.slice()` sample of full token objects when you need to confirm the reference chain. See `references/design-token-system.md`.
- **`addTheme()` takes an OBJECT, not positional args:** the high-level overview says `addTheme(group: {group, name})` but the actual runtime signature is `tok.addTheme({group: 'Color theme', name: 'Forest'})` — passing two strings `addTheme('Color theme', 'Forest')` fails with `Value not valid: <first-char-of-name>`. See `references/plugin-api-design-workflow.md`.
- **Stale shape references from `findShape`/`findShapeById`:** `board.appendChild(child)` silently no-ops when `board` was obtained via `findShape`/`findShapeById` in a PRIOR `execute_code` call — the returned object is a snapshot, not a live DOM reference. Shapes created and appended to it land on `penpot.root` instead. **Fix:** create the board AND its children in the SAME `execute_code` call, keeping the `board` variable in scope. If you must add to an existing board across calls, reparent from root: `board.appendChild(orphan); penpotUtils.setParentXY(orphan, orphan.x - board.x, orphan.y - board.y)`. See `references/plugin-api-design-workflow.md`.
- **Batch library-color modification times out:** updating many `library.local.colors` at once cascades to every component that references each color (724 components × N colors = thousands of updates). Batches of 3-5 colors per `execute_code` call complete within the 120s task limit; batches of 34+ time out. See `references/component-template-workflow.md`.
- **Export (`export_shape`) times out on heavy files:** files with 724+ components can't export a full page or even a single component within the 120s MCP task limit. The export service returns `http error` or times out. Verify color changes programmatically (`shape.fills[0].fillColor`) instead of via PNG export when the file is this heavy.
...
- `references/design-token-system.md` — the 3-tier token architecture (raw → theme → semantic), the Tokens starter kit's 14 sets + 8 themes, the reference-chain syntax (`{accent.800}`), the token-dump timeout pitfall, and the daisyUI-to-Penpot token mapping workflow for building a custom design system on the template.
- `references/component-template-workflow.md` — the component-rich template workflow (724 pre-built components, 34 library colors, no token system): inspection, batch-size-safe color remapping, font swap, verification without PNG export, and when to choose component-template vs token-system.
