...
### `<Link>` Does NOT Forward `class` Prop

**The built-in `<Link>` component silently swallows the `class` attribute.** Styled `<Link>` buttons render as invisible/unstyled text, making navigation appear completely broken.

**Always use `<a href="/routeAlias" class="... inline-block">` for styled navigation buttons.** The `inline-block` is required — `<a>` tags are inline by default, so UnoCSS `bg-*` / `px-*` utilities won't render without it. See [pattern-presentation-navigation](references/pattern-presentation-navigation.md) for full templates.
...
| MCP setup for Hermes | Config, tools, pitfalls for Hermes Agent | [mcp-setup-hermes](references/mcp-setup-hermes.md) |
...
|---------|-------------|-----------|
| Teaching from Obsidian | Convert vault notes to teaching slides with bilingual content, layout mapping, and multi-file project structure | [teaching-from-obsidian](references/teaching-from-obsidian.md) |
| Navigation & Styling | Styled buttons, cross-file `src:` imports, topic card grids, corner nav | [pattern-presentation-navigation](references/pattern-presentation-navigation.md) |
